...
bool baseConverter::baseCheck()
{
    ...
    if (charBase > 'Z' || charBase <'2') return false;
...
string baseConverter::getString(int number)
{
    return 'A' + to_string(number);
}
...
{
    bool isLargeBase = false;
    if (base > 36) isLargeBase = true;
    int countAfterDot = 0;
    ...
    string res = "";
    while (dec > 0) {
        if (isLargeBase) res = getString(mod(dec, base)) + res;
        else res = getChar(mod(dec, base)) + res;
    ...
    while (temp != 0 && countAfterDot < 10) {               // chỉ lấy 10 số sau thập phân
        temp = temp * base;
        if(isLargeBase) res += getString((int)temp);
        else res += getChar((int)temp);
        temp = mod(temp, 1);
        countAfterDot++;
    }
    this->base = base;
